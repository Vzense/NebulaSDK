...
VzenseManager::VzenseManager(std::string ip, std::string camera_name) :
        Node(camera_name),
        camera_ip_(ip),    
...
GET:
    ...
    if (status != VzReturnStatus::VzRetOK)
    {
        RCLCPP_INFO(this->get_logger(), "VZ_GetDeviceInfoList failed! %d" ,status);
        return false;
    }
    ...
    status = VZ_OpenDeviceByAlias(pPsDeviceInfo->alias, &deviceHandle_);
    if (status != VzReturnStatus::VzRetOK)
    {
