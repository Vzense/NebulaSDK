...
GET:
	...
	*/
	...
	//set new ColorGain
	float colorGain = 3.5;
	status = VZ_SetColorGain(deviceHandle, colorGain);
	if (status != VzReturnStatus::VzRetOK)
	{
		cout << "VZ_SetColorGain failed status:" << status << endl;
		return -1;
	}
	else
	{
	...
	colorGain = 0.0;
	status = VZ_GetColorGain(deviceHandle, &colorGain);
	if (status != VzReturnStatus::VzRetOK)
	{
		cout << "VZ_GetColorGain failed status:" << status << endl;
		return -1;
	}
	else
	{
	...
	params = { VzExposureControlMode_Auto , 3000};
	status = VZ_SetExposureTime(deviceHandle, VzColorSensor, params);
	if (status != VzReturnStatus::VzRetOK)
	{
